    ...
    while (std::getline(ss, line)) {
        ...
        switch (entry.backend_kind) {
            ...
            case BackendKind::Psx:
                // PSX main RAM is exposed from address 0x00000000.
                trigger.offset = static_cast<std::size_t>(absolute_address & 0x001FFFFFu);
                break;
            default: // Snes
...
bool ExperimentalRumbleManager::find_matching_user_profile(BackendKind backend_kind,
    ...
    switch (backend_kind) {
        ...
        case BackendKind::Psx:     system_name = "psx";     break;
