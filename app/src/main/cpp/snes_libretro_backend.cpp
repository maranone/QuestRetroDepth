...
// Forward declaration — defined in SPC_DSP.cpp (SNES SPC700 DSP emulator)
extern "C" void snes_set_channel_volume(int channel, float volume);
...
// snes9x's core state (Memory/CPU/PPU/DMA tables etc.) is process-global, not
// per-instance. Repeatedly calling retro_deinit()+retro_init() to "reset" it
// between ROM loads is not something the core reliably survives — doing so
// left the memory-map tables pointing at stale/freed buffers, crashing with
// SIGSEGV inside S9xGetByte the next time retro_run() walked them. Real
// libretro frontends call retro_init() exactly once per process and only
// unload/reload games afterward, so do the same here.
...
double SnesLibretroBackend::frame_rate_hz() const {
...
bool SnesLibretroBackend::load_content(const std::string& rom_path, std::string& error_out) {
    ...
    __android_log_print(ANDROID_LOG_INFO, kLogTag,
                        "Snes9x AV: fps=%.4f sample_rate=%.1f geometry=%ux%u",
                        m_frame_rate_hz,
    ...
    if (!m_frame.zbuffer.empty()) {
        ...
        if (zsrc && zstride > 0) {
            ...
            for (int li = 0; li < SNES9X_LAYER_COUNT; ++li) {
                ...
                unsigned lstride = 0;
                const uint16_t* lpix  = snes9x_get_layer_pixels(li, data, &lstride);
                const uint8_t*  lmask = snes9x_get_layer_mask(li, data, nullptr);
...
void SnesLibretroBackend::apply_controller_ports() {
    if (!m_core_initialized) return;
    retro_set_controller_port_device(0, RETRO_DEVICE_JOYPAD);
    retro_set_controller_port_device(1, m_gun_mode
        ? RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_LIGHTGUN, m_gun_peripheral)
        : RETRO_DEVICE_JOYPAD);
    // Second gun: snes9x's "Justifier (2P)" (lightgun subclass 2), which the
    // core only accepts on port 2 and then reads through port 2's lightgun
    // inputs while port 1 holds the first Justifier. Super Scope
    // (m_gun_peripheral 0) has no second gun, so nothing is plugged in there.
    const bool second_gun = m_gun_mode && m_dual_gun_mode && m_gun_peripheral == kSnesGunJustifier;
    retro_set_controller_port_device(
        2, second_gun ? RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_LIGHTGUN, 2) : RETRO_DEVICE_NONE);
}

void SnesLibretroBackend::set_gun_mode(bool enabled, int peripheral) {
    m_gun_mode = enabled;
    m_gun_peripheral = peripheral;
    apply_controller_ports();
}

void SnesLibretroBackend::set_dual_gun_mode(bool enabled) {
    if (m_dual_gun_mode == enabled) return;
    m_dual_gun_mode = enabled;
    apply_controller_ports();
}

void SnesLibretroBackend::soft_reset() {
...
void SnesLibretroBackend::set_channel_volume(int channel, float volume) {
    snes_set_channel_volume(channel, volume);
}
...
int16_t SnesLibretroBackend::handle_input_state(unsigned port, unsigned device, unsigned index, unsigned id) const {
    // Port 2 is the second Justifier (see apply_controller_ports()). It answers
    // only while that gun is really plugged in and aiming, and reports itself
    // offscreen otherwise so the game never sees a phantom player two parked
    // at the centre of the screen.
    if (port == 2 && device == RETRO_DEVICE_LIGHTGUN) {
    ...
    if (port == 1 && device == RETRO_DEVICE_LIGHTGUN) {
        ...
        // Cursor is Super Scope's secondary/"pause menu" button -- reuse reload for it.
        case RETRO_DEVICE_ID_LIGHTGUN_CURSOR: return (m_input.gun_active && m_input.gun_reload) ? 1 : 0;
        // The Justifier reads Start from the gun itself (JUSTIFIER_START), and
        // it is how Lethal Enforcers starts a game with a gun in hand.
