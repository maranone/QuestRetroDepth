...
PanelMetrics panel_metrics(PanelKind kind) {
    ...
    case PanelKind::SidePanelBar: return {1280, 200, 0.70f, 0.70f * (200.0f / 1280.0f)};
    case PanelKind::BgColorPanel: return {1024, 1280, 0.82f, 0.82f * (1280.0f / 1024.0f)};
    case PanelKind::ThemesPanel: return {1024, 1536, 0.82f, 1.23f};
    case PanelKind::Credits:  return {1024, 1536, 0.80f, 1.20f};
    }
    return {};
}
...
PanelLayout make_credits_layout(int item_count) {
    PanelLayout layout;
    layout.kind = PanelKind::Credits;
    if (item_count <= 0) return layout;
    const auto m = panel_metrics(layout.kind);
    const float title_v = 88.0f / (float)m.tex_h;
    const float row_h = (1.0f - title_v) / (float)item_count;
...
PanelLayout make_layers_layout(int layer_count, bool has_filter_row) {
    ...
    const auto m = panel_metrics(layout.kind);
    // Each layer occupies 2 slots: the existing name/visibility/ambilight row, plus a compact
    // second row for the real-geometry mode toggle and distance/thickness +/- controls. Trailing
    // rows (play/pause, auto-dup, reset-depths, filter) also take 2 slots each so their absolute
    // height on screen is unchanged from before this row split.
    const int trailing_rows = 3 + (has_filter_row ? 1 : 0);
    const int total = layer_count * 2 + trailing_rows * 2;
    if (total <= 0) return layout;
    const float title_v = 88.0f / (float)m.tex_h;
    const float row_h = std::max((1.0f - title_v) / (float)total, 28.0f / (float)m.tex_h);
    // Top-right corner of the title bar itself: cycles the Neo Geo layer-
    // composition mode (Deque/HardQ/Occup/DenseR/MinGate/TileOcc/TileOccPer/
    // TileOccGraph/StableGraph/Painter/Counter). Only drawn/hit
    // for Neo Geo -- Kotlin skips rendering the pill when the mode label is
    // empty, but the hit region is harmless to leave registered regardless.
    layout.items.push_back({{0.66f, 0.0f, 1.00f, title_v}, -1, -1, PanelRole::LayerModeToggle});
    for (int i = 0; i < layer_count; ++i) {
        const float v0p = title_v + (2 * i) * row_h;
        const float v1p = v0p + row_h;
        layout.items.push_back({{0.0f, v0p, 1.0f, v1p}, i, i, PanelRole::Row});
        layout.items.push_back({{0.60f, v0p, 0.80f, v1p}, i, i, PanelRole::Visibility});
        layout.items.push_back({{0.80f, v0p, 0.90f, v1p}, i, i, PanelRole::Ambilight});
        layout.items.push_back({{0.90f, v0p, 1.00f, v1p}, i, i, PanelRole::SideColor});

        const float v0s = v1p;
        const float v1s = v0s + row_h;
        layout.items.push_back({{0.00f, v0s, 0.28f, v1s}, i, i, PanelRole::GeometryMode});
        layout.items.push_back({{0.30f, v0s, 0.36f, v1s}, i, i, PanelRole::DistMinus});
        layout.items.push_back({{0.62f, v0s, 0.68f, v1s}, i, i, PanelRole::DistPlus});
        layout.items.push_back({{0.70f, v0s, 0.76f, v1s}, i, i, PanelRole::ThickMinus});
        layout.items.push_back({{0.92f, v0s, 0.98f, v1s}, i, i, PanelRole::ThickPlus});
    }
    const int base = layer_count * 2;
    for (int k = 0; k < trailing_rows; ++k) {
        const int row = layer_count + k; // matches existing row-index scheme used by callers (n, n+1, n+2, n+3)
        const PanelRole row_role = (k == 2) ? PanelRole::ResetDepths : PanelRole::Row;
        const float v0 = title_v + (base + 2 * k) * row_h;
        const float v1 = v0 + 2 * row_h;
        layout.items.push_back({{0.0f, v0, 1.0f, v1}, row, row, row_role});
    }
    return layout;
}
...
PanelLayout make_settings_layout(int row_count) {
    ...
    for (int i = 0; i < row_count; ++i) {
        ...
        if (i == 18) {
            // Side Panels: 5 direct-select buttons filling the row (id = side_panel_mode value)
            // instead of a whole-row cycle-on-click — there's room for all of them here.
            constexpr int kSidePanelButtonCount = 6;
            for (int b = 0; b < kSidePanelButtonCount; ++b) {
                const float u0 = (float)b / (float)kSidePanelButtonCount;
                const float u1 = (float)(b + 1) / (float)kSidePanelButtonCount;
                layout.items.push_back({{u0, title_v + i * row_h, u1, title_v + (i + 1) * row_h}, i, b, PanelRole::SidePanelSelect});
            }
        }
    }
    return layout;
...
PanelLayout make_bg_color_layout() {
    PanelLayout layout;
    layout.kind = PanelKind::BgColorPanel;
    const auto m = panel_metrics(layout.kind);
    const float title_v = 88.0f / (float)m.tex_h;
    // 4x4 grid: row 0-1 = 8 solid colors, row 2-3 = 8 gradients (id 0-7 / 8-15).
    constexpr int kCols = 4;
    constexpr int kRows = 4;
    const float cell_w = 1.0f / (float)kCols;
    const float cell_h = (1.0f - title_v) / (float)kRows;
    for (int r = 0; r < kRows; ++r) {
        for (int c = 0; c < kCols; ++c) {
            const int id = r * kCols + c;
            const float u0 = c * cell_w, u1 = (c + 1) * cell_w;
            const float v0 = title_v + r * cell_h, v1 = title_v + (r + 1) * cell_h;
            layout.items.push_back({{u0, v0, u1, v1}, 0, id, PanelRole::BgColorSelect});
        }
    }
    return layout;
}

PanelLayout make_side_panel_bar_layout() {
    PanelLayout layout;
    layout.kind = PanelKind::SidePanelBar;
    constexpr int kButtonCount = 5; // OFF/HELP/SETT/PERF/BGC
    for (int b = 0; b < kButtonCount; ++b) {
        const float u0 = (float)b / (float)kButtonCount;
        const float u1 = (float)(b + 1) / (float)kButtonCount;
        layout.items.push_back({{u0, 0.0f, u1, 1.0f}, 0, b, PanelRole::SidePanelSelect});
    }
    return layout;
}

PanelLayout make_themes_layout() {
    PanelLayout layout;
    layout.kind = PanelKind::ThemesPanel;
    const auto m = panel_metrics(layout.kind);
    const float title_v = 88.0f / (float)m.tex_h;
    const float gap_u = 18.0f / (float)m.tex_w;
    const float gap_v = 18.0f / (float)m.tex_h;
    const float cell_w = (1.0f - gap_u * 3.0f) * 0.5f;
    const float cell_h = (1.0f - title_v - gap_v * 3.0f) * 0.5f;
    for (int id = 0; id < 4; ++id) {
        const int col = id % 2;
        const int row = id / 2;
        const float u0 = gap_u + col * (cell_w + gap_u);
        const float v0 = title_v + gap_v + row * (cell_h + gap_v);
        layout.items.push_back({{u0, v0, u0 + cell_w, v0 + cell_h}, row, id, PanelRole::ThemeSelect});
    }
    return layout;
}
