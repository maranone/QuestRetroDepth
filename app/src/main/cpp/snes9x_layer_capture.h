...
// video_data is the pointer the core handed to retro_video_refresh; the
// returned pointer is biased to the same first row, exactly as the z-buffer
// and visible-source accessors do, so overscan-cropped frames stay aligned.
...
// or no tile at all).  Same stride and row bias as the pixel buffer.
