...
//     [0]  all 5 bools packed: layers_3d(1)+upscale_enabled(2)+ambilight(4)+passthrough(8)+depthmap_legacy(16)
//              → value 0–31, fits in base-36
//              NOTE: upscale is a 1-bit "on/off" flag here — UpscaleMode::PixelArt
//              and ::Fsr are indistinguishable through share codes; both decode as
//              PixelArt. Only the settings panel/ini round-trip preserves FSR.
    ...
    int layer_filter_mode = -1)
{
    ...
    int flags = (vs.layers_3d  ?  1 : 0)
              | (vs.upscale_mode != UpscaleMode::Off ?  2 : 0)
              | (vs.ambilight  ?  4 : 0)
              | (vs.shadows    ?  8 : 0)
    ...
    int* layer_filter_mode = nullptr)
{
    ...
    vs.layers_3d    = (gc &  1) != 0;
    // Share-code format only budgets 1 bit for upscale (see file header) — FSR
    // and Pixel Art both decode to Pixel Art; only "off" is distinguishable.
    vs.upscale_mode = (gc & 2) != 0 ? UpscaleMode::PixelArt : UpscaleMode::Off;
    vs.ambilight    = (gc &  4) != 0;
    vs.shadows      = (gc &  8) != 0;
    ...
    if (new_no_filter || new_with_filter) {
        ...
        if (depth_mode < 0 || depth_mode > 5) return false;
