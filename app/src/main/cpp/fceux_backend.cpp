...
// NES true independent per-layer captures: 0=BG (+backdrop), 1=sprites.
constexpr int kNesLayerCount = 2;
...
bool FceuxBackend::load_content(const std::string& rom_path, std::string& error_out) {
    ...
    // FCEUmm's controller setup consults the loaded game's input metadata.
    // It must not be called from ensure_core_initialized(), before
    // retro_load_game() has created that state.
    m_loaded_rom_path = rom_path;
    m_game_loaded = true;
    fceux_retro_set_controller_port_device(1, m_gun_mode
        ? RETRO_DEVICE_SUBCLASS(RETRO_DEVICE_MOUSE, 0) // RETRO_DEVICE_ZAPPER
        : RETRO_DEVICE_JOYPAD);
    ...
    if (!m_preview_mode || m_preview_allow_audio) {
...
// FCEUmm's libretro glue (third_party/fceumm/src/drivers/libretro/libretro.c)
// has no frameskip-related core option at all -- confirmed by grepping it for
// "frameskip"/"frame_skip". Genuinely a no-op: there is nothing to wire here,
// unlike MgbaBackend/PceBackend/SaturnLibretroBackend, whose cores do expose
// one.
...
void FceuxBackend::handle_video_frame(
    ...
    // Fill true independent per-layer RGBA buffers (0 = BG incl. backdrop,
    // 1 = sprites) from fceux_layer_capture's two-pass capture -- BG was
    // snapshotted before CopySprites() overwrote it, so an occluded BG pixel
    // survives here even though it's gone from the composited src/vsid data
    // above. Finalize converts the raw palette-index snapshots to RGBA.
    fceux_lc_finalize_frame();
    ...
    for (int li = 0; li < kNesLayerCount; ++li) {
        ...
        unsigned cap_w = 0, cap_h = 0;
        const uint32_t* cap = fceux_lc_get_layer_rgba(li, &cap_w, &cap_h);
        if (!cap || cap_w < width || cap_h < height) {
            dst.clear();
            m_frame.layers[li].depth_map.clear();
            continue;
        }
        if (dst.size() != npix) dst.resize(npix);
        const unsigned h_off = (cap_w - width) / 2;
        const unsigned v_off = (cap_h - height) / 2;
        for (unsigned y = 0; y < height; ++y) {
            std::memcpy(dst.data() + static_cast<std::size_t>(y) * width,
                        cap + static_cast<std::size_t>(y + v_off) * cap_w + h_off,
                        static_cast<std::size_t>(width) * sizeof(uint32_t));
        }
        if (li == 1) {
            // Sprite layer: populate Y-depth map from opaque pixels, same
            // convention as picodrive_backend.cpp's SMS sprite layer.
    ...
    // Throttled diagnostics for the NES color/layer path. This distinguishes
    // a bad FCEUmm frame from a bad QRD per-layer presentation without adding
    // per-frame log spam.
    if (m_video_frame_count <= 3 || (m_video_frame_count % 120u) == 0u) {
        auto color_stats = [](const std::vector<uint32_t>& pixels,
                              uint64_t& opaque, uint64_t& sum_r,
                              uint64_t& sum_g, uint64_t& sum_b) {
            opaque = sum_r = sum_g = sum_b = 0;
        ...
        uint64_t comp_opaque = 0, comp_r = 0, comp_g = 0, comp_b = 0;
        uint64_t bg_opaque = 0, bg_r = 0, bg_g = 0, bg_b = 0;
        uint64_t spr_opaque = 0, spr_r = 0, spr_g = 0, spr_b = 0;
        color_stats(m_frame.rgba8888, comp_opaque, comp_r, comp_g, comp_b);
        if (m_frame.layers.size() >= 2) {
            color_stats(m_frame.layers[0].rgba, bg_opaque, bg_r, bg_g, bg_b);
            color_stats(m_frame.layers[1].rgba, spr_opaque, spr_r, spr_g, spr_b);
        }
        const auto avg = [](uint64_t sum, uint64_t count) -> unsigned {
            return count ? static_cast<unsigned>(sum / count) : 0u;
        };
        __android_log_print(ANDROID_LOG_INFO, kLogTag,
            "FCEUmm colors frame=%llu fmt=%d mask=0x%X composite=%llu/%u,%u,%u bg=%llu/%u,%u,%u sprites=%llu/%u,%u,%u",
            static_cast<unsigned long long>(m_video_frame_count),
            static_cast<int>(m_pixel_format), m_layer_capture_mask,
            static_cast<unsigned long long>(comp_opaque),
            avg(comp_r, comp_opaque), avg(comp_g, comp_opaque), avg(comp_b, comp_opaque),
            static_cast<unsigned long long>(bg_opaque),
            avg(bg_r, bg_opaque), avg(bg_g, bg_opaque), avg(bg_b, bg_opaque),
            static_cast<unsigned long long>(spr_opaque),
            avg(spr_r, spr_opaque), avg(spr_g, spr_opaque), avg(spr_b, spr_opaque));
    }
}
...
void FceuxBackend::set_gun_mode(bool enabled, int /*peripheral*/) {
    m_gun_mode = enabled;
