#pragma once

#include <algorithm>
#include <vector>

enum class PanelKind {
    MainMenu,
    QuickEdit,
    Browser,
    Layers,
    Settings,
    SaveStates,
    Code,
    CtrlMap,
    Help,
    Homebrew,
    DashboardLeft,
    SidePanelBar, // always-visible bottom bar: 5 buttons (Off/Help/Settings/Perf/BG Color)
    BgColorPanel, // 8 solid colors + 8 gradient (floor/sky) presets
    ThemesPanel,  // four UI theme choices
    Credits,      // scrollable list of credited projects/links
};

enum class PanelRole {
    None,
    Row,
    Minus,
    Plus,
    Visibility,
    Ambilight,
    SideColor,      // cycles a layer's side/back face color override (Ori/Black/White/Red/Green/Blue)
    SaveLoadSlot,
    SaveSaveSlot,
    SaveAutosaveOption,
    SaveAutoloadOption,
    Key,
    CodeCancel,
    CodeSpace,
    CodeConfirm,
    QuickSettingsPreset,
    QuickSettingsSave,
    QuickLayersPreset,
    QuickLayersSave,
    QuickResetSettings,
    QuickResetLayers,
    QuickManualEdit,
    QuickManualVisual,
    QuickManualLayers,
    ResetDepths,
    GeometryMode,
    DistMinus,
    DistPlus,
    ThickMinus,
    ThickPlus,
    SidePanelSelect, // id = target VrState::side_panel_mode value (see openxr_shell.cpp)
    BgColorSelect,   // id = target VrState::bg_preset_index (0-7 solid, 8-15 gradient)
    ThemeSelect,     // id = UiThemeId value
    LayerModeToggle, // Neo Geo only: cycles the layer-composition algorithm
};

struct PanelMetrics {
    int   tex_w = 0;
    int   tex_h = 0;
    float world_w = 0.0f;
    float world_h = 0.0f;
};

struct UvRect {
    float u0 = 0.0f;
    float v0 = 0.0f;
    float u1 = 0.0f;
    float v1 = 0.0f;

    bool contains(float u, float v) const {
        return u >= u0 && u <= u1 && v >= v0 && v <= v1;
    }
};

struct PanelLayoutItem {
    UvRect rect;
    int row = -1;
    int id = -1;
    PanelRole role = PanelRole::None;
};

struct PanelLayout {
    PanelKind kind = PanelKind::MainMenu;
    std::vector<PanelLayoutItem> items;

    const PanelLayoutItem* hit(float u, float v) const;
    const PanelLayoutItem* row_item(int row) const;
};

PanelMetrics panel_metrics(PanelKind kind);
PanelLayout make_main_menu_layout(int item_count);
PanelLayout make_quick_edit_layout(int settings_preset_count, int layer_preset_count);
PanelLayout make_browser_layout(int visible_count, int scroll_offset);
PanelLayout make_layers_layout(int layer_count, bool has_filter_row);
PanelLayout make_settings_layout(int row_count);
PanelLayout make_save_state_layout();
PanelLayout make_code_layout();
PanelLayout make_ctrlmap_layout(int button_count, int action_count);
PanelLayout make_homebrew_layout(int entry_count, int view);
PanelLayout make_manual_dashboard_left_layout();
PanelLayout make_side_panel_bar_layout();
PanelLayout make_bg_color_layout();
PanelLayout make_themes_layout();
PanelLayout make_credits_layout(int item_count);
