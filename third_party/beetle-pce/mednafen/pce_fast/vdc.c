...
#ifdef PCE_QRD_LAYER_CAPTURE
                     {
                        int pce_row = (int)frame_counter - 14;
                        pce_lc_capture_line(pce_row, (int)target_offset, (int)width,
                                            (const uint8_t*)(bg_linebuf + (vdc->BG_XOffset & 7) + source_offset),
                                            (const uint16_t*)(spr_linebuf + 0x20 + source_offset),
                                            (unsigned)(vdc->CR & 0xC0));
                     }
#endif
                  }
