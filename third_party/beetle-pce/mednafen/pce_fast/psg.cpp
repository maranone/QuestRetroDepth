...
void PCEFast_PSG::SetChannelVolume(int channel, float volume)
{
    // Clamp channel to [0, 5]
    ...
    this->channel[channel].channel_volume = volume;
}
...
void PCEFast_PSG::UpdateOutput_Norm(const int32 timestamp, psg_channel *ch)
{
   ...
   // Apply per-channel float volume (0.0..1.0)
   delta0 = (int32)(delta0 * ch->channel_volume);
   delta1 = (int32)(delta1 * ch->channel_volume);
...
void PCEFast_PSG::Power(const int32 timestamp)
{
   ...
   for(int ch = 0; ch < 6; ch++)
   {
      ...
      channel[ch].channel_volume = 1.0f;
