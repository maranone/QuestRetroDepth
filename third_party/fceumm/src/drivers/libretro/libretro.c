...
/* Exposes the NES game palette used for the composited frame, so
 * fceux_layer_capture.cpp can convert independently-captured raw BG/sprite
 * palette indices into real colors. Entries 0..255 are frontend/UI colors;
 * the actual 512-entry NES palette starts at 256. */
uint32_t fceux_lc_palette_lookup(uint8_t index) {
   return retro_palette[256 + (index & 0x3Fu)];
}
#endif
