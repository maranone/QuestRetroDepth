...
#ifdef FCEUMM_QRD_LAYER_CAPTURE
	// Snapshot the BG-only scanline BEFORE CopySprites() overwrites pixels
	// where a sprite draws on top -- without this, an occluded BG pixel's
	// color is gone forever once CopySprites() runs, and slicing the final
	// composited frame by visible_source_id afterward can't recover it
	// (the "occlusion hole" bug -- see fceux_layer_capture.h).
	if (scanline < 240) fceux_lc_capture_bg_line(scanline, target, 256, (uint8_t)(Pal[0] & 0x3f));
#endif
	if (SpriteON)
		CopySprites(target);
