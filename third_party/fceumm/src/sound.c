...
/* Per-channel volume controls (0.0..1.0, default 1.0 = full volume)
 * Channel order: 0=Pulse1, 1=Pulse2, 2=Triangle, 3=Noise, 4=DMC */
static volatile float nes_channel_volumes[5] = { 1.0f, 1.0f, 1.0f, 1.0f, 1.0f };
...
void RDoPCM(void) {
	...
	for (V = ChannelBC[4]; V < SOUNDTS; V++)
		...
		WaveHi[V] += (int32)((double)((RawDALatch << 16) / 256) * FSettings.PCMVolume * nes_channel_volumes[4]) & (~0xFFFF);
...
static INLINE void RDoSQ(int x) {
	...
	!lengthcount[x]) {
		...
		while (V > 0) {
			if (currdc < rthresh)
				*D += (int32)((float)amp * nes_channel_volumes[x]);
			rc--;
			if (!rc) {
				rc = cf;
				currdc = (currdc + 1) & 7;
...
static void RDoSQLQ(void) {
	...
	for (x = 0; x < 2; x++) {
		...
		/* Apply per-channel volume multiplier */
		amp[x] = (int32)((float)amp[x] * nes_channel_volumes[x]);
...
static void RDoTriangle(void) {
	...
	if (!lengthcount[2] || !TriCount) {	/* Counter is halted, but we still need to output. */
		...
		while (count--) {
			*start += (int32)((double)(tcout / 256 * FSettings.TriangleVolume) * nes_channel_volumes[2]) & (~0xFFFF);  /* TODO OPTIMIZE ME */
			start++;
		}
		...
		for (V = ChannelBC[2]; V < SOUNDTS; V++) {
			WaveHi[V] += (int32)((double)(tcout / 256 * FSettings.TriangleVolume) * nes_channel_volumes[2]) & (~0xFFFF);  /* TODO OPTIMIZE ME! */
			wlcount[2]--;
			if (!wlcount[2]) {
...
static void RDoTriangleNoisePCMLQ(void) {
	...
	/* Apply per-channel volume multiplier for triangle */
	amptab[0] = (int32)((float)amptab[0] * nes_channel_volumes[2]);
	...
	{
		int32 tri_scaled = (int32)((double)lq_tcout * nes_channel_volumes[2]);
		int32 noise_scaled = (int32)((double)noiseout * nes_channel_volumes[3]);
		int32 pcm_scaled = (int32)((double)RawDALatch * nes_channel_volumes[4]);
		totalout = wlookup2[tri_scaled + noise_scaled + pcm_scaled];
	}
	...
	* TODO: Optimize this. */
	...
	if (PSG[0xE] & 0x80) {/* "short" noise */
		for (V = ChannelBC[3]; V < SOUNDTS; V++) {
			WaveHi[V] += (int32)((double)outo * nes_channel_volumes[3]);
			wlcount[3]--;
			if (!wlcount[3]) {
				uint8 feedback;
				if (PAL)
...
/* Set the per-channel volume multiplier for independent channel control.
...
void nes_set_channel_volume(int channel, float volume) {
	if (channel < 0 || channel > 4) return;  /* Silently ignore invalid channel */

	/* Clamp volume to [0.0, 1.0] */
	if (volume < 0.0f) volume = 0.0f;
	if (volume > 1.0f) volume = 1.0f;

	nes_channel_volumes[channel] = volume;
}
