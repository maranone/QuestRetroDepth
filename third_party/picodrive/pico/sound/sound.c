...
// Independent audio volume controls for FM and PSG channels
static volatile float g_genesis_fm_volume = 1.0f;
static volatile float g_genesis_psg_volume = 1.0f;
...
PICO_INTERNAL void PsndDoPSG(int cyc_to)
{
  ...
  // Apply PSG volume scaling if not at default
  float psg_vol = g_genesis_psg_volume;
  if (psg_vol != 1.0f) {
    s16 *buf = PicoIn.sndOut + pos;
    int scale_len = len << stereo;
    for (int i = 0; i < scale_len; i++) {
      buf[i] = (s16)(buf[i] * psg_vol);
    }
  }
}
...
PICO_INTERNAL void PsndDoSMSFM(int cyc_to)
{
  ...
  if (Pico.m.hardware & PMS_HW_FMUSED) {
    ...
    float fm_vol = g_genesis_fm_volume;
    if (stereo)
      while (len--) {
        *buf++ += (s16)(*buf32 * fm_vol);
        *buf++ += (s16)(*buf32++ * fm_vol);
      }
    else
      while (len--) {
...
PICO_INTERNAL void PsndDoFM(int cyc_to)
{
  ...
  if (PicoIn.opt & POPT_EN_FM) {
    ...
    // Apply FM volume scaling if not at default
    float fm_vol = g_genesis_fm_volume;
    if (fm_vol != 1.0f) {
      s32 *buf = PsndBuffer + pos;
      int scale_len = len << stereo;
      for (int i = 0; i < scale_len; i++) {
        buf[i] = (s32)(buf[i] * fm_vol);
      }
    }
  }
}
...
static int PsndRender(int offset, int length)
{
  ...
  if (length-fmlen > 0 && PicoIn.sndOut) {
    ...
    if (PicoIn.opt & POPT_EN_FM) {
      ...
      float fm_vol = g_genesis_fm_volume;
      if (fm_vol != 1.0f) {
        int scale_len = (length-fmlen) << stereo;
        for (int i = 0; i < scale_len; i++) {
          fmbuf[i] = (s32)(fmbuf[i] * fm_vol);
        }
      }
    }
  }
...
// Set independent volume for FM or PSG channels
// channel: 0 = FM (YM2612/YM2413), 1 = PSG (SN76489)
// volume: 0.0 to 1.0 (clamped), 1.0 = unchanged
void genesis_set_channel_volume(int channel, float volume)
{
  ...
  if (channel == 0) {
    g_genesis_fm_volume = volume;
  } else if (channel == 1) {
    g_genesis_psg_volume = volume;
  }
