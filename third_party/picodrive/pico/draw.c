...
static inline void rd_store_pixel(int bucket, int x, int y, unsigned char pal_index)
{
  if ((unsigned)bucket >= RD_GEN_BUCKETS) return;
...
static inline void rd_capture_packed_tile(int bucket, int dx, int y,
  unsigned int pack, unsigned char pal, int xflip)
{
  int base_x;
  unsigned char t;
  ...
  base_x = dx - 8;
  if (!xflip) {
    t = (pack&0x0000f000)>>12; if (t) rd_store_pixel(bucket, base_x + 0, y, pal|t);
    t = (pack&0x00000f00)>> 8; if (t) rd_store_pixel(bucket, base_x + 1, y, pal|t);
    t = (pack&0x000000f0)>> 4; if (t) rd_store_pixel(bucket, base_x + 2, y, pal|t);
    t = (pack&0x0000000f)    ; if (t) rd_store_pixel(bucket, base_x + 3, y, pal|t);
    t = (pack&0xf0000000)>>28; if (t) rd_store_pixel(bucket, base_x + 4, y, pal|t);
    t = (pack&0x0f000000)>>24; if (t) rd_store_pixel(bucket, base_x + 5, y, pal|t);
    t = (pack&0x00f00000)>>20; if (t) rd_store_pixel(bucket, base_x + 6, y, pal|t);
    t = (pack&0x000f0000)>>16; if (t) rd_store_pixel(bucket, base_x + 7, y, pal|t);
  } else {
    t = (pack&0x000f0000)>>16; if (t) rd_store_pixel(bucket, base_x + 0, y, pal|t);
    t = (pack&0x00f00000)>>20; if (t) rd_store_pixel(bucket, base_x + 1, y, pal|t);
    t = (pack&0x0f000000)>>24; if (t) rd_store_pixel(bucket, base_x + 2, y, pal|t);
    t = (pack&0xf0000000)>>28; if (t) rd_store_pixel(bucket, base_x + 3, y, pal|t);
    t = (pack&0x0000000f)    ; if (t) rd_store_pixel(bucket, base_x + 4, y, pal|t);
    t = (pack&0x000000f0)>> 4; if (t) rd_store_pixel(bucket, base_x + 5, y, pal|t);
    t = (pack&0x00000f00)>> 8; if (t) rd_store_pixel(bucket, base_x + 6, y, pal|t);
    t = (pack&0x0000f000)>>12; if (t) rd_store_pixel(bucket, base_x + 7, y, pal|t);
  }
}

static inline int rd_bucket_for_plane_tile(int plane_lflags, u32 code)
{
  if (plane_lflags & LF_PLANE)
    return (code & 0x8000) ? RD_GEN_PLANE_B_HIGH : RD_GEN_PLANE_B_LOW;
  return (code & 0x8000) ? RD_GEN_PLANE_A_HIGH : RD_GEN_PLANE_A_LOW;
}

static inline int rd_bucket_for_window_tile(int prio)
{
  return prio ? RD_GEN_PLANE_A_HIGH : RD_GEN_PLANE_A_LOW;
}

static inline int rd_bucket_for_sprite_code(u32 code)
{
  return (code & 0x8000) ? RD_GEN_SPR_HIGH : RD_GEN_SPR_LOW;
}

static inline void rd_begin_frame(struct PicoEState *est)
{
  int li;
  ...
  for (li = 0; li < RD_GEN_BUCKETS; li++) {
    ...
    memset(s_rd_gen_layers[li], 0, s_rd_gen_width * s_rd_gen_height * sizeof(s_rd_gen_layers[li][0]));
    memset(s_rd_gen_layer_indices[li], 0, s_rd_gen_width * s_rd_gen_height * sizeof(s_rd_gen_layer_indices[li][0]));
  }
}

static inline void rd_finalize_scanline(struct PicoEState *est, int line, int width)
{
  int li, x;

  if (!rd_capture_enabled()) return;
  if ((unsigned)line >= s_rd_gen_height) return;
  if ((unsigned)width > s_rd_gen_width) width = (int)s_rd_gen_width;

  for (li = 0; li < RD_GEN_BUCKETS; li++) {
    const unsigned base = (unsigned)line * s_rd_gen_width;
    if ((s_rd_gen_capture_mask & (1u << li)) == 0) continue;
    for (x = 0; x < width; x++) {
...
{
  s_rd_gen_capture_mask = mask & ((1u << RD_GEN_BUCKETS) - 1u);
}

uint32_t picodrive_rd_get_capture_mask(void)
{
  return s_rd_gen_capture_mask;
}

int picodrive_rd_get_layer_count(void)
{
  return RD_GEN_BUCKETS;
}
...
{
  u32 back = bgc;
  const int width = (est->Pico->video.reg[12] & 1) ? 320 : 256;
  ...
  if (rd_capture_enabled() && (s_rd_gen_capture_mask & (1u << RD_GEN_BG)) != 0 &&
    ...
    unsigned char *dst = s_rd_gen_layer_indices[RD_GEN_BG] + (unsigned)est->DrawScanline * s_rd_gen_width;
    int x;
    for (x = 0; x < width && (unsigned)x < s_rd_gen_width; x++)
      dst[x] = (unsigned char)(bgc | (sh << 7));
  }
}
#endif
...
void FinalizeLine555(int sh, int line, struct PicoEState *est)
{
  ...
  rd_finalize_scanline(est, line, len);
