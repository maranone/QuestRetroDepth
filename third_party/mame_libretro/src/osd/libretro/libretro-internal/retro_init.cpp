...
bool auto_frameskip_enable = false;
static int g_game_orientation = 0;

extern "C" int mame_game_orientation()
{
   return g_game_orientation;
}
...
{
   // The in-process RetroDepth frontend uses this to apply the same driver
   // orientation to named captures that MAME applies to its final framebuffer.
   g_game_orientation = gameRot;
   int screenRot = 0;
   bool norotate = false;
...
static void Set_Default_Option(void)
{
   ...
   // MAME's automatic frameskip controller only runs while throttling is
   // enabled. QuestRetroDepth normally paces the emulation thread itself,
   // but auto-frameskip still needs MAME's throttle flag to adjust its level.
   if (throttle_enable || auto_frameskip_enable)
      Add_Option("-throttle");
   else
      Add_Option("-nothrottle");

   // QuestRetroDepth owns this option and supplies it through the libretro
   // environment before each retro_load_game(). Keep the disabled form
   // explicit so a user's MAME config cannot silently turn it back on.
   if (auto_frameskip_enable)
   {
      Add_Option("-autoframeskip");
      // MAME uses the frameskip option as the maximum level while automatic
      // frameskip is active. Zero would silently cap auto mode at no skips.
