...
#include "../../../emu/mame_retrodepth_occupancy.h"
...
static void check_variables(void)
{
   ...
   var.key   = CORE_NAME "_autoframeskip";
   var.value = NULL;
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      if (!strcmp(var.value, "disabled"))
         auto_frameskip_enable = false;
      if (!strcmp(var.value, "enabled"))
         auto_frameskip_enable = true;
   }
...
void retro_run(void)
{
   ...
   /* RetroDepth generic MAME fallback: reset the shared draw-path capture
    * before this libretro frame starts. The commit happens after MAME has
    * finished rendering and immediately before the frontend sees the video. */
   mame_retrodepth_occupancy_detail::begin_frame((uint32_t)fb_width, (uint32_t)fb_height);
   ...
   mame_retrodepth_occupancy_detail::commit_frame();
...
bool retro_load_game(const struct retro_game_info *info)
{
   if (retro_load_ok)
   {
      /* RetroDepth: a previous game is still loaded/initialized in this
       * process. MAME's boot path (mmain2 -> cli_frontend::execute ->
       * start_luaengine) assumes it is creating a brand-new
       * mame_machine_manager/lua_engine every time -- calling it again
       * without tearing the previous one down first leaves stale global
       * Lua/sol2 state around, which reliably aborts inside sol2's
       * usertype registration by the 2nd/3rd such call. retro_finish()
       * (normally only reached via retro_deinit() at full process
       * shutdown) does that teardown (running_machine::exit +
       * mame_machine_manager destruction, which frees its lua_engine and
       * closes its lua_State) -- run it here too so retro_load_game() is
       * safe to call more than once per process, matching how MAME's own
       * in-UI game switch (retro_main_loop()'s retro_execute() path)
       * already behaves. */
      retro_finish();
   }

   /* RetroDepth: retro_pause is only ever reset to 0 inside retro_init()
    * (called once per process). retro_unload_game() sets it to -1 and
    * nothing else clears it, so after the first unload/reload cycle every
    * subsequent game stays permanently paused -- retro_run()'s
    * `if (!retro_pause) retro_main_loop();` then never advances the
    * simulation at all, and every frame callback just re-emits whatever
    * was on screen at the moment of the previous unload. This is what
    * showed up as background-cached thumbnails for every ROM after the
    * first one in a session being frozen on one static (often black
