...
void gp9001vdp_device::render_vdp(bitmap_ind16 &bitmap, const rectangle &cliprect, bitmap_ind8 &primap)
{
	...
	// One insertion point here covers every toaplan2-era driver that shares
	// this VDP device (batsugun/dogyuun/truxton2/fixeight/vfive/snowbro2/
	// kbash/raizing_batrider/etc) with zero per-driver changes. Each of the
	// 3 tilemap layers and sprites are re-drawn once more into private
	// scratch buffers + a scratch priority map so the real primap (already
	// consumed above) is untouched.
	if (retrodepth_active())
	{
		const int bw = bitmap.width(), bh = bitmap.height();

		static bitmap_ind16 s_l0, s_l1, s_l2, s_sprites;
		static bitmap_ind8 s_pri;
		if (s_l0.width() != bw || s_l0.height() != bh) {
			s_l0.allocate(bw, bh);
			s_l1.allocate(bw, bh);
			s_l2.allocate(bw, bh);
			s_sprites.allocate(bw, bh);
			s_pri.allocate(bw, bh);
		}

		s_l0.fill(0xffff, cliprect); s_pri.fill(0, cliprect);
		draw_custom_tilemap(s_l0, cliprect, s_pri, 0);

		s_l1.fill(0xffff, cliprect); s_pri.fill(0, cliprect);
		draw_custom_tilemap(s_l1, cliprect, s_pri, 1);

		s_l2.fill(0xffff, cliprect); s_pri.fill(0, cliprect);
		draw_custom_tilemap(s_l2, cliprect, s_pri, 2);

		s_sprites.fill(0xffff, cliprect); s_pri.fill(0, cliprect);
		draw_sprites(s_sprites, cliprect, s_pri);
		...
		const int w = cliprect.width(), h = cliprect.height();
		...
		auto write_layer = [&](uint32_t z_order, const char *name, bitmap_ind16 &bmp, bool has_owners) {
			for (int row = 0; row < h; ++row) {
				for (int col = 0; col < w; ++col) {
					uint16_t pen = bmp.pix(cliprect.min_y + row, cliprect.min_x + col);
					int idx = row * w + col;
		...
		write_layer(0, "layer0", s_l0, false);
		write_layer(1, "layer1", s_l1, true);
		write_layer(2, "layer2", s_l2, true);
