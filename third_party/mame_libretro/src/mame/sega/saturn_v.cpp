...
uint32_t saturn_state::screen_update_vdp2(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	// --- RetroDepth Saturn source-plane export ---
	// Saturn has two cooperating video processors rather than one simple
	// tilemap stack. Export the VDP2 source planes independently and keep all
	// VDP1 sprites/polygons together. The ordinary m_tmpbitmap composition below
	// remains the reference image; this export is intentionally source-based so
	// a missing/unused plane is transparent instead of duplicating the frame.
	if (retrodepth_active())
	{
		const rectangle &vis = screen.visible_area();
		const int bw = m_tmpbitmap.width(), bh = m_tmpbitmap.height();
		static bitmap_rgb32 s_back, s_nbg0, s_nbg1, s_nbg2, s_nbg3;
		static bitmap_rgb32 s_rbg0, s_rbg1, s_vdp1;
		if (s_back.width() != bw || s_back.height() != bh)
		{
			s_back.allocate(bw, bh);
			s_nbg0.allocate(bw, bh);
			s_nbg1.allocate(bw, bh);
			s_nbg2.allocate(bw, bh);
			s_nbg3.allocate(bw, bh);
			s_rbg0.allocate(bw, bh);
			s_rbg1.allocate(bw, bh);
			s_vdp1.allocate(bw, bh);
		}

		const rgb_t transparent = rgb_t::transparent();
		s_back.fill(transparent, cliprect);
		s_nbg0.fill(transparent, cliprect);
		s_nbg1.fill(transparent, cliprect);
		s_nbg2.fill(transparent, cliprect);
		s_nbg3.fill(transparent, cliprect);
		s_rbg0.fill(transparent, cliprect);
		s_rbg1.fill(transparent, cliprect);
		s_vdp1.fill(transparent, cliprect);

		vdp2_draw_back(s_back, cliprect);
		if (VDP2_DISP)
		{
			// NBG0 is also Saturn's RBG1 source when rotation screen 1 is on.
			if (VDP2_R1ON)
				vdp2_draw_NBG0(s_rbg1, cliprect);
			else
				vdp2_draw_NBG0(s_nbg0, cliprect);
			vdp2_draw_NBG1(s_nbg1, cliprect);
			vdp2_draw_NBG2(s_nbg2, cliprect);
			vdp2_draw_NBG3(s_nbg3, cliprect);
			vdp2_draw_RBG0(s_rbg0, cliprect);

			// draw_sprites() reads the already-rendered VDP1 framebuffer and
			// selects one VDP2 sprite priority per call. Calling it for every
			// priority into one transparent bitmap preserves the complete VDP1
			// pass without pretending each sprite is a hardware plane.
			for (uint8_t pri = 1; pri < 8; ++pri)
				draw_sprites(s_vdp1, cliprect, pri);
		}
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			auto write_layer = [&](uint32_t z_order, const char *name, bitmap_rgb32 &src)
			{
				for (int row = 0; row < h; ++row)
				{
					for (int col = 0; col < w; ++col)
					{
						const rgb_t pixel = src.pix(vis.min_y + row, vis.min_x + col);
						packed[(size_t)row * w + col] = pixel.a() == 0
							? 0x00000001u : uint32_t(pixel);
					}
				}
				retrodepth_write_layer(z_order, name, packed.data(), nullptr,
					(uint32_t)w, (uint32_t)h);
			};

			write_layer(0, "saturn_back",  s_back);
			write_layer(1, "saturn_nbg0",  s_nbg0);
			write_layer(2, "saturn_nbg1",  s_nbg1);
			write_layer(3, "saturn_nbg2",  s_nbg2);
			write_layer(4, "saturn_nbg3",  s_nbg3);
			write_layer(5, "saturn_rbg0",  s_rbg0);
			write_layer(6, "saturn_rbg1",  s_rbg1);
			write_layer(7, "saturn_vdp1",  s_vdp1);
			retrodepth_commit();
		}
	}
