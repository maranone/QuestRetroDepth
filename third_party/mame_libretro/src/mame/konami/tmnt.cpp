...
uint32_t tmnt_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		m_k052109->tilemap_draw(screen, s_bg, cliprect, 2, TILEMAP_DRAW_OPAQUE, 0);

		s_mid.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k052109->tilemap_draw(screen, s_mid, cliprect, 1, 0, 0);

		s_fg.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k052109->tilemap_draw(screen, s_fg, cliprect, 0, 0, 0);

		s_sprites.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k051960->k051960_sprites_draw(s_sprites, cliprect, screen.priority(), 0, 0);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "scroll2", s_bg,      false);
			write_layer(1, "scroll1", s_mid,     true);
			write_layer(2, "scroll0", s_fg,      true);
