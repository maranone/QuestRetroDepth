...
uint32_t lethal_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg3, s_bg2, s_bg1, s_sprites, s_text;
		if (s_bg3.width() != bw || s_bg3.height() != bh) {
		...
		s_bg3.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k056832->tilemap_draw(screen, s_bg3, cliprect, 3, K056832_DRAW_FLAG_MIRROR, 1);

		s_bg2.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k056832->tilemap_draw(screen, s_bg2, cliprect, 2, K056832_DRAW_FLAG_MIRROR, 2);

		s_bg1.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k056832->tilemap_draw(screen, s_bg1, cliprect, 1, K056832_DRAW_FLAG_MIRROR, 4);

		s_sprites.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k053244->sprites_draw(s_sprites, cliprect, screen.priority());

		s_text.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k056832->tilemap_draw(screen, s_text, cliprect, 0, K056832_DRAW_FLAG_MIRROR, 0);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			auto write_layer = [&](uint32_t z_order, const char* name, bitmap_ind16& bmp) {
			...
			write_layer(0, "lethal_bg3",     s_bg3);
			write_layer(1, "lethal_bg2",     s_bg2);
			write_layer(2, "lethal_bg1",     s_bg1);
			write_layer(3, "lethal_sprites", s_sprites);
			write_layer(4, "lethal_text",    s_text);
