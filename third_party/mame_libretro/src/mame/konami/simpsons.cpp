...
Notes:
      ...
      horizontal: 16 cycles front porch, 32 cycles sync, 16 cycles back porch
                  (yes, visible width really is 320 pixels)
...
uint32_t simpsons_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_layer[3], s_sprites;
		if (s_layer[0].width() != bw || s_layer[0].height() != bh) {
			for (int i = 0; i < 3; ++i) s_layer[i].allocate(bw, bh);
			s_sprites.allocate(bw, bh);
		}

		// Render tilemaps in sorted order: layer[0]=background, layer[2]=foreground
		for (int i = 0; i < 3; ++i) {
			s_layer[i].fill(0xffff, cliprect);
			screen.priority().fill(0, cliprect);
			m_k052109->tilemap_draw(screen, s_layer[i], cliprect, layer[i], 0, 0);
		}

		s_sprites.fill(0xffff, cliprect);
		screen.priority().fill(0, cliprect);
		m_k053246->k053247_sprites_draw(s_sprites, cliprect);

		if (cliprect.max_y >= vis.max_y)
		{
			// simpsons_state has no m_palette device handle (only a tag-based
			// "palette" share) -- reach it via the screen's own palette instead.
			const rgb_t* colors = screen.palette().palette()->entry_list_adjusted();
			const int n_pens = (int)screen.palette().palette()->num_colors();
			...
			// layer[0]=background tilemap, layer[2]=foreground tilemap
			write_layer(0, "scroll2", s_layer[0], true);
			write_layer(1, "scroll1", s_layer[1], true);
			write_layer(2, "scroll0", s_layer[2], true);
			write_layer(3, "sprites", s_sprites,  true);
...
void simpsons_state::simpsons(machine_config &config)
{
	...
	ymsnd.add_route(0, "speaker", 0.5, 0); // only left channel is connected
	ymsnd.add_route(0, "speaker", 0.5, 1);
	ymsnd.add_route(1, "speaker", 0.0, 0);
	ymsnd.add_route(1, "speaker", 0.0, 1);

	k053260_device &k053260(K053260(config, "k053260", 3.579545_MHz_XTAL)); // verified on pcb
	k053260.add_route(0, "speaker", 0.5, 0);
	k053260.add_route(1, "speaker", 0.5, 1);
