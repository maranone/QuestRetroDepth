...
uint32_t nycaptor_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// Simplified 3-layer split: the runtime spot()&3-switched interleave and
	// 4-priority-band structure above are collapsed into one composite
	// redraw per logical layer (LAYER0 tilemap plane, LAYER1 tilemap plane,
	// all sprite priority bands), same class of simplification as every
	// other RetroDepth hook's fixed z-order.
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg0, s_bg1, s_sprites;
		...
		for (int pri = 0; pri < 4; ++pri)
			m_bg_tilemap->draw(screen, s_bg0, cliprect, TILEMAP_DRAW_LAYER0 | pri, 0);
		for (int pri = 0; pri < 4; ++pri)
			m_bg_tilemap->draw(screen, s_bg1, cliprect, TILEMAP_DRAW_LAYER1 | pri, 0);
		for (int pri = 0; pri < 8; ++pri)
			draw_sprites(s_sprites, cliprect, pri);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "nycaptor_bg0",     s_bg0,     false);
			write_layer(1, "nycaptor_bg1",     s_bg1,     true);
			write_layer(2, "nycaptor_sprites", s_sprites, true);
