...
/* we actually assume text layer is on top of everything anyway, but FWIW... */
	...
	*/
	...
	// Same TC0480SCP scheme as gunbustr.cpp; the "puck" priority exception
	// documented above is ignored here, same class of simplification as
	// every other RetroDepth hook's fixed z-order.
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg0, s_bg1, s_bg2, s_bg3, s_text, s_sprites;
		if (s_bg0.width() != bw || s_bg0.height() != bh) {
			s_bg0.allocate(bw, bh);
			s_bg1.allocate(bw, bh);
			s_bg2.allocate(bw, bh);
			s_bg3.allocate(bw, bh);
			s_text.allocate(bw, bh);
			s_sprites.allocate(bw, bh);
		}

		s_bg1.fill(kTransSentinel, cliprect);
		s_bg2.fill(kTransSentinel, cliprect);
		s_bg3.fill(kTransSentinel, cliprect);
		s_text.fill(kTransSentinel, cliprect);
		s_sprites.fill(kTransSentinel, cliprect);

		m_tc0480scp->tilemap_draw(screen, s_bg0, cliprect, layer[0], TILEMAP_DRAW_OPAQUE, 1);
		m_tc0480scp->tilemap_draw(screen, s_bg1, cliprect, layer[1], 0, 2);
		m_tc0480scp->tilemap_draw(screen, s_bg2, cliprect, layer[2], 0, 4);
		m_tc0480scp->tilemap_draw(screen, s_bg3, cliprect, layer[3], 0, 8);
		m_tc0480scp->tilemap_draw(screen, s_text, cliprect, layer[4], 0, 0);
		draw_sprites(screen, s_sprites, cliprect, primasks, 0);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "tc0480_bg0",     s_bg0,     false);
			write_layer(1, "tc0480_bg1",     s_bg1,     true);
			write_layer(2, "tc0480_bg2",     s_bg2,     true);
			write_layer(3, "tc0480_bg3",     s_bg3,     true);
			write_layer(4, "tc0480_sprites", s_sprites, true);
			write_layer(5, "tc0480_text",    s_text,    true);
