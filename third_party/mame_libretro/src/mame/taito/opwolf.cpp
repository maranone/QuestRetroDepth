...
uint32_t opwolf_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// PC080SN provides two tilemap layers (background + foreground scroll),
	// PC090OJ provides one sprite layer. Redraw each in isolation into an
	// offscreen bitmap purely for export -- the real display bitmap above is
	// untouched and still goes through the normal composited video_refresh.
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg, s_fg, s_sprites;
		...
		s_fg.fill(kTransSentinel, cliprect);
		s_sprites.fill(kTransSentinel, cliprect);

		// Background tilemap draws opaque (every pixel written), so no
		// transparency sentinel is needed for it.
		m_pc080sn->tilemap_draw(screen, s_bg, cliprect, layer[0], TILEMAP_DRAW_OPAQUE, 1);
		m_pc080sn->tilemap_draw(screen, s_fg, cliprect, layer[1], 0, 2);
		m_pc090oj->draw_sprites(screen, s_sprites, cliprect);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "taito_bg",      s_bg,       false);
			write_layer(1, "taito_fg",      s_fg,       true);
			write_layer(2, "taito_sprites", s_sprites,  true);
...
GAME( 1987, opwolfb,  opwolf,   opwolfb,  opwolfb, opwolf_state,  init_opwolfb,  ROT0, "bootleg (Bear)", "Operation Bear (bootleg of Operation Wolf)", MACHINE_IMPERFECT_SOUND | MACHINE_SUPPORTS_SAVE )
