...
// full rationale) for the Taito PC080SN/PC090OJ family (opwolf, othunder,
// undrfire, ...). Header-only + `inline` so every family's copy of this
// file can coexist in the same build without a duplicate-symbol error --
// only one MAME driver is ever loaded/running at a time, so sharing the
// same global state across families is harmless.
//
