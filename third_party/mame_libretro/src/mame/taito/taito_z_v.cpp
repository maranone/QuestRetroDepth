...
u32 spacegun_state::screen_update_spacegun(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg0, s_bg1, s_text, s_sprites;
		...
		m_tc0100scn->tilemap_draw(screen, s_bg0, cliprect, layer[0], TILEMAP_DRAW_OPAQUE, 1);
		m_tc0100scn->tilemap_draw(screen, s_bg1, cliprect, layer[1], 0, 2);
		m_tc0100scn->tilemap_draw(screen, s_text, cliprect, layer[2], 0, 4);
		spacegun_draw_sprites_16x8(screen, s_sprites, cliprect, 4);

		if (cliprect.max_y >= vis.max_y)
		{
			const int n_pens = (int)m_tc0110pcr->entries();

			static uint32_t pal_data[256 * 16];
			memset(pal_data, 0, sizeof(pal_data));
			int copy_count = (n_pens < 256 * 16) ? n_pens : (256 * 16);
			for (int i = 0; i < copy_count; ++i)
				pal_data[i] = (uint32_t)m_tc0110pcr->pen_color(i);
				...
				for (int row = 0; row < h; ++row) {
					for (int col = 0; col < w; ++col) {
						...
						if (has_trans && pen == kTransSentinel) {
							...
							packed[idx] = (pen < (uint16_t)n_pens) ? (uint32_t)m_tc0110pcr->pen_color(pen) : 0u;
			...
			write_layer(0, "spacegun_bg0",     s_bg0,     false);
			write_layer(1, "spacegun_bg1",     s_bg1,     true);
			write_layer(2, "spacegun_sprites", s_sprites, true);
			write_layer(3, "spacegun_text",    s_text,    true);
