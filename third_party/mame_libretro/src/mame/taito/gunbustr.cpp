...
u32 gunbustr_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		m_tc0480scp->tilemap_draw(screen, s_bg0, cliprect, layer[0], TILEMAP_DRAW_OPAQUE, 0);
		m_tc0480scp->tilemap_draw(screen, s_bg1, cliprect, layer[1], 0, 1);
		m_tc0480scp->tilemap_draw(screen, s_bg2, cliprect, layer[2], 0, 2);
		m_tc0480scp->tilemap_draw(screen, s_bg3, cliprect, layer[3], 0, 4);
		m_tc0480scp->tilemap_draw(screen, s_text, cliprect, layer[4], 0, 8);
		draw_sprites(screen, s_sprites, cliprect, primasks, 48, -116);
