...
uint32_t lordgun_base_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// Piggybacks m_bitmaps[0..4] -- the driver already isolates every layer
	// into its own buffer above (pri*0x800+pen packed pens) before the
	// per-pixel priority composite, so no redundant redraw is needed here.
	if (retrodepth_active())
	{
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			auto write_layer = [&](uint32_t z_order, const char* name, int layer_idx) {
				for (int row = 0; row < h; ++row) {
					for (int col = 0; col < w; ++col) {
						const uint16_t raw = m_bitmaps[layer_idx].pix(vis.min_y + row, vis.min_x + col);
						const bool transparent = raw == trans_pen;
						const uint16_t pen = raw & 0x7ff;
						int idx = row * w + col;
						if (transparent) {
			...
			write_layer(0, "lordgun_tile0",   0);
			write_layer(1, "lordgun_tile1",   1);
			write_layer(2, "lordgun_tile2",   2);
			write_layer(3, "lordgun_tile3",   3);
			write_layer(4, "lordgun_sprites", 4);
