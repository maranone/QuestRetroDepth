...
// questretrodepth.
//
// Desktop RetroDepth's MAME patches (retrodepth/mame-patches/) call a
// shared-memory version of this exact API ("Local\RetroDepthFrameBuffer",
// see retrodepth/mame-patches/src/emu/retrodepth.h) because there MAME runs
// in its own process (retrodepth.exe reads the shmem). On Android, MAME runs
// in-process inside questretrodepth's own mame_libretro.so on its emulator
// thread, so this header keeps the identical function signatures the
// CPS1/CPS2/Konami/NeoGeo driver patches already call (write_layer/commit/
// palette route/etc.) but backs them with plain in-process buffers instead
// of shared memory -- the driver .cpp patch files themselves need no changes
// beyond including this header.
//
// Header-only + `inline`: MAME's genie/Makefile.libretro build resolves its
// driver source list dynamically from SOURCES=capcom/cps1.cpp,capcom/cps2.cpp
// (see questretrodepth's build notes), so there is no straightforward way to
// register a brand-new standalone .cpp translation unit into that build.
// Living entirely in a header both driver files #include sidesteps that
// entirely -- inline linkage lets the (identical) definitions in cps1_v.cpp
// and cps2.cpp's translation units coexist without a duplicate-symbol error.
//
...
// Debug-only: which of the 256 palette banks had at least one active sprite
// this frame (see neosprite_base_device::rd_compute_slot_depths()), for the
// Neo Geo palette-swatch capture tool (questretrodepth's
// neogeo_palette_debug.cpp). Single-buffered -- a torn frame here is
// harmless, this is diagnostic tooling, not the real render path.
inline uint8_t  g_active_palettes[256] = {};
inline uint32_t g_active_palette_count = 0;
...
// Counter-mode frame rejection: discard the write set without flipping it,
// so the renderer continues reading the last committed good layer frame.
// The next emulator frame will reuse this now-empty write set normally.
inline void retrodepth_discard_write() {
    using namespace mame_retrodepth_detail;
    g_sets[g_write_idx].clear();
    g_zbufs[g_write_idx].z.clear();
    g_zbufs[g_write_idx].width = g_zbufs[g_write_idx].height = 0;
...
inline void retrodepth_write_active_palettes(const uint8_t* pals, uint32_t count) {
    using namespace mame_retrodepth_detail;
    if (count > 256) count = 256;
    if (count) std::memcpy(g_active_palettes, pals, count);
    g_active_palette_count = count;
}
...
// Debug-only palette accessors (see neogeo_palette_debug.cpp on the app side).
RD_EXPORT const uint32_t* mame_palette_argb_table() {
    return mame_retrodepth_detail::g_palette_argb;
}

RD_EXPORT uint32_t mame_active_palette_count() {
    return mame_retrodepth_detail::g_active_palette_count;
}

RD_EXPORT const uint8_t* mame_active_palette_list() {
    return mame_retrodepth_detail::g_active_palettes;
}
