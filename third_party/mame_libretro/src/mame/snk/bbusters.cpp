...
uint32_t bbusters_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// m_bitmap_sprites[0]/[1] are already isolated by the normal path above
	// (filled/drawn before this point), so they're reused directly here
	// instead of redrawing. The palette-bank priority split on sprite chip 1
	// (the "elevator stage" case noted above) is simplified away into one
	// bbusters_sprites1 layer, same class of simplification as every other
	// RetroDepth hook's fixed z-order.
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_pf1, s_pf0, s_fix;
		if (s_pf1.width() != bw || s_pf1.height() != bh) {
			s_pf1.allocate(bw, bh);
			s_pf0.allocate(bw, bh);
			s_fix.allocate(bw, bh);
		}

		s_pf0.fill(kTransSentinel, cliprect);
		s_fix.fill(kTransSentinel, cliprect);

		m_pf_tilemap[1]->draw(screen, s_pf1, cliprect, 0, 0);
		m_pf_tilemap[0]->draw(screen, s_pf0, cliprect, 0, 0);
		m_fix_tilemap->draw(screen, s_fix, cliprect, 0, 0);

		if (cliprect.max_y >= vis.max_y)
		{
			palette_device* pal = subdevice<palette_device>("palette");
			const rgb_t* colors = pal->palette()->entry_list_adjusted();
			const int n_pens = (int)pal->palette()->num_colors();
			...
			write_layer(0, "bbusters_pf1",      s_pf1,               false, false);
			write_layer(1, "bbusters_pf0",      s_pf0,               true,  false);
			write_layer(2, "bbusters_sprites1", m_bitmap_sprites[1], false, true);
			write_layer(3, "bbusters_sprites0", m_bitmap_sprites[0], false, true);
			write_layer(4, "bbusters_fix",      s_fix,               true,  false);
