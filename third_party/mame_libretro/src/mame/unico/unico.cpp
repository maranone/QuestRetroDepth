...
uint32_t burglarx_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (retrodepth_active())
	{
		...
		m_tilemap[0]->draw(screen, s_bg,      cliprect, 0, 1);
		m_tilemap[1]->draw(screen, s_mid,     cliprect, 0, 2);
		m_tilemap[2]->draw(screen, s_fg,      cliprect, 0, 4);
		draw_sprites(screen, s_sprites, cliprect);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "unico_bg",       s_bg,      false);
			write_layer(1, "unico_mid",      s_mid,     true);
			write_layer(2, "unico_fg",       s_fg,      true);
			write_layer(3, "unico_sprites",  s_sprites, true);
