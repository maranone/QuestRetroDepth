...
u32 oneshot_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// draw_crosshairs() below is a UI/debug overlay, not game content -- it
	// runs after this block so the export bitmaps never see it.
	if (retrodepth_active())
	{
		...
		static bitmap_ind16 s_bg, s_mid, s_sprites, s_fg;
		...
		s_bg.fill(m_palette->black_pen(), cliprect);
		s_mid.fill(kTransSentinel, cliprect);
		s_sprites.fill(kTransSentinel, cliprect);
		s_fg.fill(kTransSentinel, cliprect);

		m_bg_tilemap->draw(screen, s_bg, cliprect, 0, 0);
		m_mid_tilemap->draw(screen, s_mid, cliprect, 0, 0);
		draw_sprites(s_sprites, cliprect);
		m_fg_tilemap->draw(screen, s_fg, cliprect, 0, 0);
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			write_layer(0, "oneshot_bg",      s_bg,      false);
			write_layer(1, "oneshot_mid",     s_mid,     true);
			write_layer(2, "oneshot_sprites", s_sprites, true);
			write_layer(3, "oneshot_fg",      s_fg,      true);
