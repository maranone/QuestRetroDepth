...
uint32_t x1_020_dx_101_device::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	// This hardware has no tilemaps at all (sprite-only), and `bitmap` above
	// already holds exactly that single composited layer, so export it
	// directly instead of redrawing into an isolated buffer. Shared by every
	// game using this device -- harmless for ones RetroDepth doesn't map to
	// a multi-layer profile, since they stay on the full_frame config and
	// never read the "seta2_sprites" named layer.
		...
		if (cliprect.max_y >= vis.max_y)
		{
			...
			for (int row = 0; row < h; ++row) {
				for (int col = 0; col < w; ++col) {
					uint16_t pen = bitmap.pix(vis.min_y + row, vis.min_x + col);
					packed[row * w + col] = (pen < (uint16_t)n_pens) ? (uint32_t)colors[pen] : 0u;
				}
			}
			retrodepth_write_layer(0, "seta2_sprites", packed.data(), nullptr, w, h);
			retrodepth_commit();
		}
	}
