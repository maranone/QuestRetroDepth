...
if _OPTIONS["osd"] == "retro" then
		-- RetroDepth: without an explicit SONAME the linker has nothing to
		-- record for this .so's own identity, so anything that links against
		-- it (questretrodepth_native.so) embeds the literal absolute
		-- build-time path as its DT_NEEDED entry instead of a portable
		-- filename -- works fine on the WSL host, then fails to dlopen on
		-- the real device with an UnsatisfiedLinkError naming that path.
		-- Must match third_party/mame_libretro_prebuilt/arm64-v8a/
		-- libmame_libretro.so, the name the app's CMakeLists.txt imports it
		-- under.
		linkoptions {
			"-shared",
			"-Wl,-soname,libmame_libretro.so",
		}
else
		targetprefix "lib"
		targetname "main"
