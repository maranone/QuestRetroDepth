...
bool8 S9xGraphicsInit (void)
{
	...
	GFX.CurrentVisibleSourceId = 0xff;
	...
	GFX.VisibleSourceBuffer = (uint8 *) malloc(GFX.ScreenSize);

	if (!GFX.ZERO || !GFX.SubScreen || !GFX.ZBuffer || !GFX.SubZBuffer || !GFX.VisibleSourceBuffer)
	{
		S9xGraphicsDeinit();
		return (FALSE);
	}
...
void S9xGraphicsDeinit (void)
{
	...
	if (GFX.VisibleSourceBuffer) { free(GFX.VisibleSourceBuffer); GFX.VisibleSourceBuffer = NULL; }
}
...
void S9xStartScreenRefresh (void)
{
	...
	if (IPPU.RenderThisFrame)
	{
		...
		memset(GFX.VisibleSourceBuffer, 0xff, GFX.ScreenSize);
		snes9x_clear_layer_capture();
	}
...
static inline void RenderScreen (bool8 sub)
{
	...
	if (BGActive & 0x10)
	{
		...
		GFX.CurrentVisibleSourceId = sub ? 0xff : 4;
		// Sub-screen pixels are colour-math inputs, never main-screen content;
		// capturing them dusts the OBJ layer with pixels the player never sees.
		GFX.CaptureLayerIndex = sub ? -1 : 4;
		DrawOBJS(D + 4);
		GFX.CaptureLayerIndex = -1;
		GFX.CurrentVisibleSourceId = 0xff;
	}
	...
	#define DO_BG(n, pal, depth, hires, offset, Zh, Zl, voffoff) \
		if (BGActive & (1 << n)) \
		{ \
			GFX.CurrentVisibleSourceId = sub ? 0xff : (uint8_t)(n); \
			GFX.CaptureLayerIndex = sub ? (int8_t)-1 : (int8_t)(n); \
			BG.StartPalette = pal; \
	...
	switch (PPU.BGMode)
	{
		...
		case 7:
			if (BGActive & 0x01)
			{
				...
				GFX.CurrentVisibleSourceId = sub ? 0xff : 0;
			...
			if ((Memory.FillRAM[0x2133] & 0x40) && (BGActive & 0x02))
			{
				...
				GFX.CurrentVisibleSourceId = sub ? 0xff : 1;
	...
	GFX.CurrentVisibleSourceId = sub ? 0xff : 5;
