...
namespace TileImpl {
	...
	void Normal2x1Base<MATH, BPSTART>::Draw(int N, int M, uint32 Offset, uint32 OffsetInLine, uint8 Pix, uint8 Z1, uint8 Z2)
	{
		...
		if (Z1 > GFX.DB[Offset + 2 * N] && (M))
		{
			...
			if (GFX.VisibleSourceBuffer && GFX.CurrentVisibleSourceId != 0xff) {
				GFX.VisibleSourceBuffer[Offset + 2 * N] = GFX.CurrentVisibleSourceId;
				GFX.VisibleSourceBuffer[Offset + 2 * N + 1] = GFX.CurrentVisibleSourceId;
			}
		}
		// Per-layer capture: write every non-transparent pixel regardless of depth.
		if (Pix && GFX.CaptureLayerIndex >= 0 && !GFX.ClipColors) {
			snes9x_layer_capture_put(GFX.CaptureLayerIndex, Offset + 2*N,   GFX.RealScreenColors[Pix]);
			snes9x_layer_capture_put(GFX.CaptureLayerIndex, Offset + 2*N+1, GFX.RealScreenColors[Pix]);
		}
	}
