...
struct SGFX
{
	...
	// Per-layer capture support.  Set to 0-3 (BG index) or 4 (OBJ) before
	// each DrawBackground / DrawOBJS call so the tile renderers dual-write
	// every opaque pixel into the corresponding per-layer buffer.
	// Set to -1 when not capturing (backdrop draw, between layers, etc.).
	int8_t CaptureLayerIndex;
	// Main-screen final ownership capture.
	// 0-3 = BG0-BG3, 4 = OBJ, 5 = backdrop, 0xff = disabled/none.
	uint8_t CurrentVisibleSourceId;
};
