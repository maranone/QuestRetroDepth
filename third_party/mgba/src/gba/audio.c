...
// Independent volume controls for split audio channels
// PSG group (channels 1-4), Direct Sound A, and Direct Sound B
// Each defaults to 1.0 (unchanged output)
static volatile float g_gba_psg_volume = 1.0f;
static volatile float g_gba_dsa_volume = 1.0f;
static volatile float g_gba_dsb_volume = 1.0f;
...
void GBAAudioSample(struct GBAAudio* audio, int32_t timestamp) {
	...
	for (sample = audio->sampleIndex; timestamp >= audio->sampleInterval && sample < maxSample; ++sample, timestamp -= audio->sampleInterval) {
		...
		// Apply PSG volume control
		float psg_vol = g_gba_psg_volume;
		sampleLeft = (int16_t)(sampleLeft * psg_vol);
		sampleRight = (int16_t)(sampleRight * psg_vol);

		if (!audio->forceDisableChA) {
			if (audio->chALeft) {
				int16_t chA_sample = (int16_t)((audio->chA.samples[sample] << 2) >> !audio->volumeChA);
				float dsa_vol = g_gba_dsa_volume;
				sampleLeft += (int16_t)(chA_sample * dsa_vol);
			}

			if (audio->chARight) {
				int16_t chA_sample = (int16_t)((audio->chA.samples[sample] << 2) >> !audio->volumeChA);
				float dsa_vol = g_gba_dsa_volume;
				sampleRight += (int16_t)(chA_sample * dsa_vol);
			}
		}

		if (!audio->forceDisableChB) {
			if (audio->chBLeft) {
				int16_t chB_sample = (int16_t)((audio->chB.samples[sample] << 2) >> !audio->volumeChB);
				float dsb_vol = g_gba_dsb_volume;
				sampleLeft += (int16_t)(chB_sample * dsb_vol);
			}

			if (audio->chBRight) {
				int16_t chB_sample = (int16_t)((audio->chB.samples[sample] << 2) >> !audio->volumeChB);
				float dsb_vol = g_gba_dsb_volume;
				sampleRight += (int16_t)(chB_sample * dsb_vol);
			}
		}
...
void GBAudioSetChannelVolume(int channel, float volume) {
	// Clamp channel to valid range [0, 2]
	...
	switch (channel) {
	case 0:
		g_gba_psg_volume = volume;
		break;
	case 1:
		g_gba_dsa_volume = volume;
		break;
	case 2:
		g_gba_dsb_volume = volume;
		break;
	}
}
