...
{ \
   ...
   { \
      ...
      { \
         ...
         { \
            ...
            { \
               ...
               psx_qrd_scene_push_quad_rhi(first->precise[0], \
                  first->precise[1], first->precise[2], \
                  ...
                  gpu->min_u, gpu->min_v, gpu->max_u, gpu->max_v, \
                  gpu->TexPageX, gpu->TexPageY, clut_x, clut_y, blend_mode, \
                  2 - (MO_LIT), DitherEnabled(gpu), (BM_VAL), (ME_LIT), \
                  gpu->MaskSetOR != 0, false, gpu->may_be_2d, (TEXTURED_LIT)); \
         ...
         { \
            ...
            psx_qrd_scene_push_triangle_rhi(verts[0].precise[0], \
               verts[0].precise[1], verts[0].precise[2], \
               verts[1].precise[0], verts[1].precise[1], verts[1].precise[2], \
               verts[2].precise[0], verts[2].precise[1], verts[2].precise[2], \
               ...
               gpu_precise_tri_rgb(verts), verts[0].u, verts[0].v, \
               verts[1].u, verts[1].v, verts[2].u, verts[2].v, \
               gpu->min_u, gpu->min_v, gpu->max_u, gpu->max_v, \
               gpu->TexPageX, gpu->TexPageY, clut_x, clut_y, blend_mode, \
               2 - (MO_LIT), DitherEnabled(gpu), (BM_VAL), (ME_LIT), \
               gpu->MaskSetOR != 0, (TEXTURED_LIT)); \
      ...
      { \
         /* QRD consumes the same command stream even though this build uses
          * Beetle's software rasteriser. Keep capture independent of RHI so
          * native OpenXR mode does not require a second GL context. */ \
         psx_qrd_scene_push_triangle_rhi(vertices[0].precise[0], vertices[0].precise[1], vertices[0].precise[2], \
            vertices[1].precise[0], vertices[1].precise[1], vertices[1].precise[2], \
            vertices[2].precise[0], vertices[2].precise[1], vertices[2].precise[2], \
            ((uint32_t)vertices[0].r) | ((uint32_t)vertices[0].g << 8) | ((uint32_t)vertices[0].b << 16), \
            ((uint32_t)vertices[1].r) | ((uint32_t)vertices[1].g << 8) | ((uint32_t)vertices[1].b << 16), \
            ((uint32_t)vertices[2].r) | ((uint32_t)vertices[2].g << 8) | ((uint32_t)vertices[2].b << 16), \
            gpu_precise_tri_rgb(vertices), vertices[0].u, vertices[0].v, \
            vertices[1].u, vertices[1].v, vertices[2].u, vertices[2].v, \
            gpu->min_u, gpu->min_v, gpu->max_u, gpu->max_v, \
            gpu->TexPageX, gpu->TexPageY, clut_x, clut_y, blend_mode, \
            2 - (MO_LIT), DitherEnabled(gpu), (BM_VAL), (ME_LIT), \
            gpu->MaskSetOR != 0, (TEXTURED_LIT)); \
