...
/* QRD's optional source-layer capture categories. The PS1 GPU has no
 * dedicated text plane, Z-buffer, or 3D-geometry-vs-2D-effect split, but
 * the GP0 command opcode byte already tells us everything the hardware
 * itself distinguishes per-primitive: draw kind (background/polygon/
 * line/rectangle), texture-mapped vs flat, Gouraud vs flat shading,
 * semi-transparency blending, and (for rectangles) fixed glyph-sized
 * tiles vs larger sprites. Bits 0-1 hold the kind; bits 2-5 are
 * independent flags layered on top, so a single captured byte can
 * classify a primitive into one of 8 depth layers (see
 * classify_psx_layer() in psx_libretro_backend.cpp) without any 3D
 * geometry extraction. */
#define GPU_CAP_KIND_BG      0
#define GPU_CAP_KIND_POLY    1
#define GPU_CAP_KIND_LINE    2
#define GPU_CAP_KIND_RECT    3
#define GPU_CAP_KIND_MASK    0x3

#define GPU_CAP_FLAG_TEXTURED   (1 << 2)
#define GPU_CAP_FLAG_GOURAUD    (1 << 3)
#define GPU_CAP_FLAG_SEMITRANS  (1 << 4)
#define GPU_CAP_FLAG_SMALLRECT  (1 << 5)

/* Rectangles at or below this native pixel dimension on either axis are
 * treated as glyph/HUD-tile sized (PS1 fonts are almost always fixed-size
 * 8x8 or 16x16 tiles submitted via the GP0 fixed-size rectangle opcodes). */
#define GPU_CAP_SMALLRECT_MAX_DIM 16

/* Legacy aliases: the original 3-class scheme mapped 1:1 onto the new
 * kind field for background/polygon, with "text" meaning any rectangle. */
#define GPU_CAPTURE_BACKGROUND GPU_CAP_KIND_BG
#define GPU_CAPTURE_POLYGON    GPU_CAP_KIND_POLY
#define GPU_CAPTURE_TEXT       GPU_CAP_KIND_RECT
...
struct PS_GPU
{
   ...
   */
   ...
   /* QRD-only source-layer capture sidecars. These are runtime buffers and
    * are intentionally not part of the serialized emulation state. */
   uint8_t  *capture_tags;
   uint8_t  *capture_output_tags;
   uint32_t  capture_output_tag_bytes;
   int32_t   capture_output_tag_pitch;
   uint8_t   capture_class;
...
/* Optional source-layer capture used by the QRD frontend. The tag buffer is
 * populated at the same scanout coordinates as the software video surface,
 * so the frontend can split the final image without temporal inference. */
