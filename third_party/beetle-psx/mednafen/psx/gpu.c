...
static void Command_FBFill(PS_GPU* gpu, const uint32_t *cb)
{
   ...
   psx_qrd_scene_push_fill(cb[0], (uint16_t)destX, (uint16_t)destY,
...
static void Command_FBCopy(PS_GPU* g, const uint32_t *cb)
{
   ...
   psx_qrd_scene_push_copy((uint16_t)sourceX, (uint16_t)sourceY,
         (uint16_t)destX, (uint16_t)destY, (uint16_t)width, (uint16_t)height,
         g->MaskEvalAND != 0, g->MaskSetOR != 0);
...
static uint8_t *CaptureTags_Alloc(uint8_t upscale_shift)
{
   const unsigned width  = 1024u << upscale_shift;
   const unsigned height = 512u << upscale_shift;
   return (uint8_t *)calloc((size_t)width * height, sizeof(uint8_t));
}
      ...
      int sls, int sle, uint8_t upscale_shift)
{
   ...
   GPU.capture_tags = CaptureTags_Alloc(upscale_shift);
   if (!GPU.capture_tags)
   {
      free(GPU.vram);
      GPU.vram = NULL;
   ...
   free(GPU.capture_output_tags);
   GPU.capture_output_tags = NULL;
   GPU.capture_output_tag_bytes = 0;
   GPU.capture_output_tag_pitch = 0;
   GPU.capture_class = GPU_CAPTURE_BACKGROUND;
...
void GPU_Destroy(void)
{
   free(GPU.vram);
   GPU.vram = NULL;
   free(GPU.capture_tags);
   GPU.capture_tags = NULL;
   free(GPU.capture_output_tags);
   GPU.capture_output_tags = NULL;
   GPU.capture_output_tag_bytes = 0;
   GPU.capture_output_tag_pitch = 0;
...
bool GPU_Rescale(uint8_t ushift)
{
   ...
   uint8_t  *old_tags = GPU.capture_tags;
   ...
   new_tags = CaptureTags_Alloc(ushift);
   if (!new_tags)
   {
      free(new_vram);
      if (old_shift != 0)
   ...
   for (unsigned y = 0; y < 512; y++)
      for (unsigned x = 0; x < 1024; x++)
      {
         const uint8_t tag = old_tags
            ? old_tags[(y << (10 + old_shift)) | (x << old_shift)]
            : GPU_CAPTURE_BACKGROUND;
         for (unsigned dy = 0; dy < (1u << ushift); dy++)
            for (unsigned dx = 0; dx < (1u << ushift); dx++)
               new_tags[((y << ushift) + dy) * (1024u << ushift)
                        + (x << ushift) + dx] = tag;
      }
...
void GPU_Power(void)
{
   ...
   if (GPU.capture_tags)
      memset(GPU.capture_tags, GPU_CAPTURE_BACKGROUND,
             512 * 1024 * UPSCALE(&GPU) * UPSCALE(&GPU) * sizeof(*GPU.capture_tags));
...
/* Classify a GP0 command opcode byte into a QRD capture tag. The PS1
 * command encoding already carries every flag we need: bit1 is the
 * semi-transparency enable, bit2 is texture-mapped, bit3 is quad/4-point
 * (irrelevant to capture), bit4 is Gouraud shading for polygons/lines,
 * and bits3-4 pick the fixed rectangle size (0=variable, else <=16x16). */
static INLINE uint8_t ClassifyCaptureTag(uint32_t cc)
{
   if (cc >= 0x20 && cc <= 0x3F)
   {
      uint8_t tag = GPU_CAP_KIND_POLY;
      if (cc & 0x02) tag |= GPU_CAP_FLAG_SEMITRANS;
      if (cc & 0x04) tag |= GPU_CAP_FLAG_TEXTURED;
      if (cc & 0x10) tag |= GPU_CAP_FLAG_GOURAUD;
      return tag;
   }
   if (cc >= 0x40 && cc <= 0x5F)
   {
      uint8_t tag = GPU_CAP_KIND_LINE;
      if (cc & 0x02) tag |= GPU_CAP_FLAG_SEMITRANS;
      if (cc & 0x10) tag |= GPU_CAP_FLAG_GOURAUD;
      return tag;
   }
   if (cc >= 0x60 && cc <= 0x7F)
   {
      uint8_t tag = GPU_CAP_KIND_RECT;
      if (cc & 0x02) tag |= GPU_CAP_FLAG_SEMITRANS;
      if (cc & 0x04) tag |= GPU_CAP_FLAG_TEXTURED;
      /* Fixed-size rects (raw_size 1/2/3) are glyph/HUD-tile sized;
       * variable-size (raw_size 0) is left unflagged since its true
       * dimensions aren't known until the extra GP0 word is read. */
      if (cc & 0x18) tag |= GPU_CAP_FLAG_SMALLRECT;
      return tag;
   }
   return GPU_CAP_KIND_BG;
}
...
static void ProcessFIFO(uint32_t in_count)
{
   ...
   /* The PS1 command stream has no text opcode. Its closest source-level
    * distinction is polygon/line primitives versus rectangle/sprite
    * primitives; the latter is where glyphs and HUD elements are normally
    * submitted. Framebuffer transfers/fills stay in the background class. */
   if (GPU.InCmd == INCMD_FBWRITE)
      GPU.capture_class = GPU_CAP_KIND_BG;
   else
      GPU.capture_class = ClassifyCaptureTag(cc);
...
/* Replays only the source-address walk used by ReorderRGB_Var. Keeping this
 * separate means the existing SIMD/scalar colour conversion remains
 * untouched; when capture is enabled this writes one source-class byte per
 * output pixel using the same VRAM origin and upscale rules. */
static INLINE void CaptureScanoutTags(uint8_t *dest, const uint8_t *src_tags,
      bool bpp24, const int32_t dx_start, const int32_t dx_end, int32_t fb_x,
      unsigned upscale_shift, unsigned upscale)
{
   const int32_t fb_mask = ((0x7FF << upscale_shift) + upscale - 1);
   const unsigned row_words = 1024u << upscale_shift;
   int32_t x;

   if (!dest || !src_tags) return;
   for (x = dx_start; x < dx_end; x += (int32_t)upscale)
   {
      const unsigned word = ((unsigned)(fb_x >> 1)) & (row_words - 1u);
      const uint8_t tag = src_tags[word];
      for (unsigned i = 0; i < upscale; ++i)
         dest[x + (int32_t)i] = tag;
      fb_x = (fb_x + ((bpp24 ? 3 : 2) << upscale_shift)) & fb_mask;
   }
}
               ...
               if (rhi_intf_is_type() == RHI_SOFTWARE)
               {
                  ...
                  else
                  {
                     ...
                     for (uint32_t i = 0; i < _upscale; i++)
                     {
                        ...
                        if (GPU.capture_enabled && GPU.capture_output_tags)
                           CaptureScanoutTags(
                              GPU.capture_output_tags +
                                 ((dest_line << GPU.upscale_shift) + i) *
                                 GPU.capture_output_tag_pitch,
                              GPU.capture_tags + (y + i) *
                                 (1024u << GPU.upscale_shift),
                              GPU.DisplayMode & DISP_RGB24,
                              udx_start, udx_end, ufb_x,
                              GPU.upscale_shift, _upscale);
...
void GPU_StartFrame(EmulateSpecStruct *espec_arg)
{
   ...
   if (GPU.capture_enabled && GPU.surface)
   {
      const uint32_t bytes = (uint32_t)GPU.surface->pitch32 *
                             (uint32_t)GPU.surface->h;
      if (GPU.capture_output_tag_bytes != bytes)
      {
         uint8_t *new_tags = (uint8_t *)realloc(GPU.capture_output_tags, bytes);
         if (new_tags)
         {
            GPU.capture_output_tags = new_tags;
            GPU.capture_output_tag_bytes = bytes;
         }
      }
      GPU.capture_output_tag_pitch = GPU.surface->pitch32;
      if (GPU.capture_output_tags)
         memset(GPU.capture_output_tags, GPU_CAPTURE_BACKGROUND,
                GPU.capture_output_tag_bytes);
   }
   else
   {
      GPU.capture_output_tag_pitch = 0;
   ...
   psx_qrd_scene_begin(GPU.vram,
         1024u << GPU.upscale_shift, 512u << GPU.upscale_shift,
         GPU.upscale_shift);
}

void GPU_SetLayerCaptureEnabled(bool enabled)
{
   GPU.capture_enabled = enabled;
   GPU.capture_class = GPU_CAPTURE_BACKGROUND;
}

const uint8_t *GPU_GetLayerCaptureOutput(int *width, int *height, int *pitch)
{
   if (width)  *width  = GPU.surface ? GPU.surface->w : 0;
   if (height) *height = GPU.surface ? GPU.surface->h : 0;
   if (pitch)  *pitch  = GPU.capture_output_tag_pitch;
   if (!GPU.capture_enabled || !GPU.capture_output_tags ||
       GPU.capture_output_tag_pitch <= 0)
      return NULL;
   return GPU.capture_output_tags;
}
      ...
      for (field = 0; field < 2; field++)
      {
         ...
         for (i = 0; i < up; i++)
         {
            ...
            if (GPU.capture_enabled && GPU.capture_output_tags)
               CaptureScanoutTags(
                  GPU.capture_output_tags +
                     ((dest_line << s) + i) * GPU.capture_output_tag_pitch,
                  GPU.capture_tags + (y_up + i) * (1024u << s),
                  rec->rgb24, udx_start, udx_end, ufb_x, s, up);
