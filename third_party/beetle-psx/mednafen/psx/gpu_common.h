...
/* Set a pixel in VRAM */
#define vram_put(gpu, x, y, v) do { \
   uint32_t qrd_capture_index = ((y) << (10 + (gpu)->upscale_shift)) | (x); \
   (gpu)->vram[qrd_capture_index] = (v); \
   if ((gpu)->capture_enabled && (gpu)->capture_tags) \
      (gpu)->capture_tags[qrd_capture_index] = (gpu)->capture_class; \
} while (0)
