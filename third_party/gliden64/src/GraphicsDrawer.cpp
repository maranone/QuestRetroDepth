...
#ifdef QRD_N64_HOOK
#include "n64_gliden64_hook.h"
#ifdef __ANDROID__
#include <android/log.h>
#define QRD_G64_LOG(...) __android_log_print(ANDROID_LOG_INFO, "GLideN64Hook", __VA_ARGS__)
#else
#define QRD_G64_LOG(...) do {} while (0)
#endif
// Defined in VI.cpp and set via qrd_n64_register_hooks().
typedef void (*QrdCaptureTrianglesFn)(const QrdDrawTriangleParams*);
extern QrdCaptureTrianglesFn g_qrd_capture_triangles;
#endif

using namespace graphics;

#ifdef QRD_N64_HOOK
static void qrd_capture_triangles(SPVertex* vertices, u32 vertex_count, void* elements, u32 element_count)
{
	if (vertices == nullptr || vertex_count == 0 || element_count == 0)
		return;

	if (g_qrd_capture_triangles == nullptr) {
		static bool s_logged_missing = false;
		if (!s_logged_missing) {
			QRD_G64_LOG("qrd_n64_capture_triangles not registered");
			s_logged_missing = true;
		}
		return;
	}

	QrdDrawTriangleParams params = {};
	params.vertices = reinterpret_cast<const QrdSPVertex*>(vertices);
	params.vertex_count = vertex_count;
	params.elements = elements;
	params.element_count = element_count;
	memcpy(params.game_proj, gSP.matrix.projection, sizeof(params.game_proj));
	memcpy(params.game_combined, gSP.matrix.combined, sizeof(params.game_combined));
	params.is_billboard = gSP.matrix.billboard != 0;
	params.depth_write = gDP.otherMode.depthUpdate != 0;
	g_qrd_capture_triangles(&params);

	static u32 s_hook_draws = 0;
	++s_hook_draws;
	if (s_hook_draws <= 12 || (s_hook_draws % 500) == 0)
		QRD_G64_LOG("capture draw=%u vertices=%u elements=%u indexed=%d",
			s_hook_draws, vertex_count, element_count, elements != nullptr);
}
#endif
...
#ifdef QRD_N64_HOOK
	qrd_capture_triangles(triParams.vertices, triParams.verticesCount, triParams.elements, triParams.elementsCount);
#endif
...
void GraphicsDrawer::drawScreenSpaceTriangle(u32 _numVtx, graphics::DrawModeParam _mode)
{
	...
	triParams.combiner = currentCombiner();
#ifdef QRD_N64_HOOK
	qrd_capture_triangles(triParams.vertices, triParams.verticesCount, nullptr, triParams.verticesCount);
#endif
	gfxContext.drawTriangles(triParams);
	g_debugger.addTriangles(triParams);
	m_dmaVerticesNum = 0;
