...
#define QRD_G64_VI_LOG(...) __android_log_print(ANDROID_LOG_INFO, "GLideN64Hook", __VA_ARGS__)
#else
#define QRD_G64_VI_LOG(...) do {} while (0)
#endif

// Hook function pointers — set by qrd_n64_register_hooks() called from the host .so
// after GfxPluginStartup, avoiding dlsym(RTLD_DEFAULT) which fails in Android namespaces.
typedef void (*QrdFrameEndFn)();
typedef void (*QrdCaptureTrianglesFn)(const QrdDrawTriangleParams*);
QrdFrameEndFn         g_qrd_frame_end         = nullptr;
QrdCaptureTrianglesFn g_qrd_capture_triangles  = nullptr;

extern "C" void qrd_n64_register_hooks(void* frame_end, void* capture_triangles) {
    g_qrd_frame_end         = reinterpret_cast<QrdFrameEndFn>(frame_end);
    g_qrd_capture_triangles = reinterpret_cast<QrdCaptureTrianglesFn>(capture_triangles);
    QRD_G64_VI_LOG("qrd_n64_register_hooks: frame_end=%p capture=%p", frame_end, capture_triangles);
}
#endif
...
#ifdef QRD_N64_HOOK
	static u32 s_vi_updates = 0;
	++s_vi_updates;
	if (s_vi_updates <= 12 || (s_vi_updates % 120) == 0)
		QRD_G64_VI_LOG("VI_UpdateScreen count=%u origin=%08x width=%u",
			s_vi_updates, *REG.VI_ORIGIN, *REG.VI_WIDTH);

	if (g_qrd_frame_end != nullptr) {
		g_qrd_frame_end();
	} else {
		static bool s_logged_missing = false;
		if (!s_logged_missing) {
			QRD_G64_VI_LOG("qrd_n64_frame_end not registered");
