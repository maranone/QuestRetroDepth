...
void* HostInterface::retro_get_memory_data(unsigned id)
{
  switch (id)
  {
    ...
    case RETRO_MEMORY_VIDEO_RAM:
      if (!System::IsShutdown() && System::GetGPU())
        return const_cast<uint16_t*>(System::GetGPU()->GetVRAM());
      break;
...
size_t HostInterface::retro_get_memory_size(unsigned id)
{
  switch (id)
  {
    ...
    case RETRO_MEMORY_VIDEO_RAM:
      return System::IsShutdown() ? 0 : (VRAM_WIDTH * VRAM_HEIGHT * sizeof(uint16_t));
...
void HostInterface::UpdateControllers()
{
  ...
  for (uint32_t i = 0; i < NUM_CONTROLLER_AND_CARD_PORTS; i++)
  {
    // A port that is not a gun this frame must not leave last frame's
    // crosshair on screen (e.g. dual-gun mode turned back off).
    if (m_display && g_settings.controller_types[i] != ControllerType::NamcoGunCon)
      m_display->ClearLightgunState(i);
...
void HostInterface::UpdateControllersNamcoGunCon(uint32_t index)
{
  ...
  // Per-port aim. This is what NamcoGunCon::UpdatePosition reads during a SIO
  // Transfer to compute the gun's beam-tick / scanline pair; it is part of the
  // PSX-visible input fingerprint, so SetHostPosition raises the runahead
  // replay flag on a change (a position change with no button change would
  // otherwise leave the replayed simulation aiming at last frame's pixel).
  controller->SetHostPosition(pos_x, pos_y);
  // Player one also drives the display's global mouse position, kept for the
  // code outside the gun that still reads it. Player two must not touch it —
  // that global being last-writer-wins is exactly what made two guns share
  // one aim.
  if (index == 0)
    m_display->SetMousePosition(pos_x, pos_y);
  // Cache the normalized 16-bit gun state per port for the HW renderers. They
  ...
  m_display->SetLightgunState(index, gun_x, gun_y, offscreen);
