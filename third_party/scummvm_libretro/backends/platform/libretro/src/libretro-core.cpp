...
// RetroDepth layer export callback — set by the host before retro_run()
typedef void (*retrodepth_layer_export_fn_t)(const char *layer_id, uint32_t w, uint32_t h, const uint8_t *bgra);
retrodepth_layer_export_fn_t g_retrodepth_layer_export = nullptr;

extern "C"
#ifdef _WIN32
__declspec(dllexport)
#endif
void retrodepth_set_layer_export(retrodepth_layer_export_fn_t cb) {
    g_retrodepth_layer_export = cb;
}

// RetroDepth cursor export callback — fired each frame with the cursor sprite
// rgba is NULL when visible==false; hot_x/hot_y are unscaled sprite hotspot
typedef void (*retrodepth_cursor_export_fn_t)(int hot_x, int hot_y,
                                               uint32_t w, uint32_t h,
                                               const uint8_t *rgba, bool visible);
retrodepth_cursor_export_fn_t g_retrodepth_cursor_export = nullptr;

extern "C"
#ifdef _WIN32
__declspec(dllexport)
#endif
void retrodepth_set_cursor_export(retrodepth_cursor_export_fn_t cb) {
    g_retrodepth_cursor_export = cb;
}

// RetroDepth current-room getter — read-only, updated by ScummEngine::startScene
// (engines/scumm/room.cpp) on every room change. Lets the host pick the right
// cached per-room depth map (see tools/scummvm_scraper) without needing a
// callback registration; -1 until the first room loads.
extern "C" int g_retrodepth_current_room = -1;

extern "C"
#ifdef _WIN32
__declspec(dllexport)
#endif
int retrodepth_get_current_room() {
    return g_retrodepth_current_room;
}
