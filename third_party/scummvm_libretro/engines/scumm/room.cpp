 ...
 */
// See libretro-core.cpp: read-only room id for the RetroDepth host, so it
// can select the right cached per-room depth map (tools/scummvm_scraper).
extern "C" int g_retrodepth_current_room;

void ScummEngine::startScene(int room, Actor *a, int objectNr) {
	int i, where;

	debugC(DEBUG_GENERAL, "Loading room %d", room);
	g_retrodepth_current_room = room;
