...
typedef void (*retrodepth_layer_export_fn_t)(const char *, uint32_t, uint32_t, const uint8_t *);
extern retrodepth_layer_export_fn_t g_retrodepth_layer_export;
...
void ScummEngine::processActors() {
	...
	// --- RetroDepth: per-object layer capture ---
	VirtScreen *rdVS = &_virtscr[kMainVirtScreen];
	const bool rd_cap = g_retrodepth_layer_export
	                    && rdVS && rdVS->w > 0 && rdVS->h > 0
	                    && rdVS->format.bytesPerPixel == 1;
	const int rdW = rd_cap ? (int)rdVS->w : 0;
	const int rdH = rd_cap ? (int)rdVS->h : 0;
	std::vector<uint8_t> rd_prev, rd_buf;

	if (rd_cap) {
		// Export the room background — kMainVirtScreen before any actors are drawn.
		const byte *pix = (const byte *)rdVS->getPixels(0, 0);
		rd_buf.resize((size_t)rdW * rdH * 4);
		for (int i = 0; i < rdW * rdH; ++i) {
			rd_buf[i*4+0] = _currentPalette[pix[i]*3+2];
			rd_buf[i*4+1] = _currentPalette[pix[i]*3+1];
			rd_buf[i*4+2] = _currentPalette[pix[i]*3+0];
			rd_buf[i*4+3] = 0xFF;
		}
		g_retrodepth_layer_export("background", (uint32_t)rdW, (uint32_t)rdH, rd_buf.data());
		// Snapshot for per-actor diffing (rd_prev is always "state before this actor")
		rd_prev.assign(pix, pix + (size_t)rdW * rdH);
	}

	// Diff the VirtScreen against rd_prev to isolate this actor's pixels, then export.
	// Updates rd_prev for the next actor so each diff is independent.
	auto rd_export_actor = [&](Actor *ac) {
		if (!rd_cap) return;
		const byte *cur = (const byte *)rdVS->getPixels(0, 0);
		rd_buf.assign((size_t)rdW * rdH * 4, 0); // start transparent
		bool hit = false;
		for (int i = 0; i < rdW * rdH; ++i) {
			if (cur[i] != rd_prev[i]) {
				rd_buf[i*4+0] = _currentPalette[cur[i]*3+2];
				rd_buf[i*4+1] = _currentPalette[cur[i]*3+1];
				rd_buf[i*4+2] = _currentPalette[cur[i]*3+0];
				rd_buf[i*4+3] = 0xFF;
				hit = true;
			}
		}
		// Build "actor:N" without stdio (FILE is forbidden in ScummVM build).
		char rid[16];
		rid[0]='a'; rid[1]='c'; rid[2]='t'; rid[3]='o'; rid[4]='r'; rid[5]=':';
		int n = (int)ac->_number; char digs[8]; int nd = 0;
		do { digs[nd++] = (char)('0' + n % 10); n /= 10; } while (n > 0);
		int k = 6;
		for (int d = nd - 1; d >= 0; --d) rid[k++] = digs[d];
		rid[k] = '\0';
		if (hit) {
			g_retrodepth_layer_export(rid, (uint32_t)rdW, (uint32_t)rdH, rd_buf.data());
		} else {
			// Actor is in scene but pixels unchanged — heartbeat (w=0) keeps layer alive.
			g_retrodepth_layer_export(rid, 0, 0, nullptr);
		}
		rd_prev.assign(cur, cur + (size_t)rdW * rdH);
	};
	// --- end RetroDepth init ---
